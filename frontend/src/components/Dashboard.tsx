...
export default function Dashboard({ result }: { result: ParseResult }) {
  ...
  const segs = [
    { label: ".text", v: text, c: "var(--a)" },
    { label: ".rodata", v: rodata, c: "var(--a-dim)" },
    { label: ".data", v: data, c: "var(--b)" },
    { label: ".bss", v: bss, c: "var(--b-dim)" },
  ];
      ...
      <div className="panel">
        ...
        <div className="p-4">
          <div className="flex h-7 rounded-[2px] overflow-hidden border ln">
            {segs.map(g => g.v > 0 && (
              <div key={g.label} title={`${g.label} · ${(g.v / 1024).toFixed(2)} KB`}
                style={{ width: `${(g.v / total) * 100}%`, background: g.c, borderRight: "1px solid rgba(0,0,0,.4)" }} />
            ))}
          </div>
          <div className="flex flex-wrap gap-4 mt-3 mono text-[11px] mut">
            {segs.map(g => (
              <span key={g.label} className="flex items-center gap-1.5">
                <span style={{ width: 9, height: 9, background: g.c, display: "inline-block" }} />
                {g.label} <span className="fg">{((g.v / total) * 100).toFixed(1)}%</span>
              </span>
            ))}
          </div>
        </div>
