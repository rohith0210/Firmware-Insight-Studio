...
export default function Ribbon({ title, result, loading, accent, device, override, setOverride, cycleAccent, onJSON, onCSV, onReset }: {
  title: string; result: ParseResult | null; loading: boolean; accent: string; device: Device | null; override: string; setOverride: (s: string) => void;
  ...
  return (
    <div className="ribbon">
      ...
      {result && device && (
        <span className="chip" title="target device — override in Settings">
          <span className="mut">target</span>
          <select value={override} onChange={e => setOverride(e.target.value)} style={{ background: "transparent", border: 0, color: "var(--a)", fontFamily: "JetBrains Mono", fontSize: 11, outline: "none", cursor: "pointer" }}>
