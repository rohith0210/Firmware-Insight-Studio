...
const GROUPS: { lbl: string; items: { id: View; t: string; ic: string; locked?: boolean }[] }[] = [
  ...
  { lbl: "FIRMWARE", items: [{ id: "debug", t: "Disassembler", ic: "⌬" }, { id: "callgraph", t: "Call Graph", ic: "⑂" }, { id: "linker", t: "Linker Script", ic: "⌗", locked: true }] },
  ...
  return (
    <aside className="side">
      <div className="brand">
        ...
        <div><h1 className="fg">FIRMWARE<br />INSIGHT</h1><small className="mut">workbench v1.2</small></div>
      </div>
      {GROUPS.map(g => (
        <div className="nav-grp" key={g.lbl}>
          ...
          {g.items.map(it => (
            <div key={it.id} className={`nav-item ${view === it.id ? "on" : ""} ${!hasResult ? "off" : ""}`} onClick={() => hasResult && setView(it.id)}>
              <span className="ic">{it.ic}</span><span>{it.t}</span>
              {it.locked && <span className="soon">soon</span>}
            </div>
          ))}
        </div>
      ))}
