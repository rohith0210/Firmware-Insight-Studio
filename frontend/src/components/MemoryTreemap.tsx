...
/* ---------- squarified treemap math (Bruls et al.), zero deps ---------- */
...
function squarify(vals: number[], rect: R): R[] {
  ...
  while (i < q.length) {
    ...
    if (!rowV.length || worst([...rowV, c.v], w) <= wNow) { rowV.push(c.v); rowI.push(c); i++; }
...
const estW = (s: string, px: number) => s.length * px * 0.62;
...
export default function MemoryTreemap({ data }: { data: any[] }) {
  ...
  const { totalAll, maxLeaf } = useMemo(() => {
    const totalAll = data.reduce((a, d) => a + (d.size || 0), 0);
    let maxLeaf = 1;
    for (const d of data) for (const c of (d.children || [])) if (c.size > maxLeaf) maxLeaf = c.size;
    return { totalAll, maxLeaf };
  }, [data]);
  ...
  const view = useMemo(() => {
    ...
    if (focus) {
      ...
      if (!sec) return { mode: "zoom" as const, leafRects: [] as LR[], title: focus, total: 0, n: 0 };
      const lv = leavesOf(sec, 60);
      const rects = squarify(lv.map(l => l.size), inner);
    ...
    // ---- top level: width-floored columns; tiny sections become a labeled list, never slivers ----
    const secs = data.filter(d => d.size > 0).sort((a, b) => b.size - a.size);
    const big = secs.filter(s => s.size / totalAll >= BIG);
    const tiny = secs.filter(s => s.size / totalAll < BIG);
    ...
    const nCols = colsDef.length || 1;
    const usable = inner.w - COLGAP * (nCols - 1);
    const floor = Math.max(48, Math.min(132, usable / nCols));
    ...
    const remain = usable - fixed;
    const cols: Col[] = [];
    let cx = inner.x;
    colsDef.forEach((c, i) => {
      const w = widths[i] < 0 ? (flexSum > 0 ? (-widths[i] / flexSum) * remain : remain) : widths[i];
      const frameH = inner.h;
      const headerH = w > 150 ? 38 : w > 96 ? 30 : 0;
      if (c.kind === "sec") {
        ...
        const listTop = inner.y + headerH + LEAFGAP;
        const listH = Math.max(0, frameH - headerH - LEAFGAP * 2);
        const rowsDef = (c.secs || []).slice().sort((a, b) => b.size - a.size);
        const minR = Math.min(24, listH / Math.max(1, rowsDef.length));
  ...
  const LeafG = (l: LR, delay: number, labeled: boolean) => (
    <g key={l.id} className="tm-leaf"
      style={{ transformBox: "fill-box", transformOrigin: "center", animation: "tmIn .34s ease both", animationDelay: `${Math.min(delay, 32) * 5}ms`, opacity: hover && hover !== l.id ? 0.4 : 1, cursor: labeled ? "default" : "pointer" }}
      onMouseMove={e => showTip(e, [l.name, `${(l.size / 1024).toFixed(2)} KB`, `${pct(l.size, l.secSize)}% of ${l.secName}`, `${pct(l.size, totalAll)}% of binary`])}
      onMouseLeave={() => { setHover(null); setTip(null); }}
      onMouseEnter={() => setHover(l.id)}
      onClick={() => { if (!labeled) setFocus(l.secName); }}>
      <rect x={l.x} y={l.y} width={Math.max(0, l.w - LEAFGAP)} height={Math.max(0, l.h - LEAFGAP)} rx={3} fill={l.fill} stroke={hover === l.id ? "#e8f1ec" : "rgba(0,0,0,.35)"} strokeWidth={hover === l.id ? 1.4 : 0.6} />
    ...
    <div className="panel" style={{ position: "relative" }}>
      ...
      <div className="panel-head">
        ...
        <span className="flex items-center gap-3">
          ...
          <span className="tag">{focus ? "functions · hover for size" : "hover = names · click a section to zoom"}</span>
        </span>
      </div>
      <div className="p-3">
        <div ref={ref} style={{ width: "100%", height: H }}>
          <svg width="100%" height={H} style={{ display: "block" }} onMouseLeave={() => setTip(null)}>
            {view.mode === "zoom" ? (
              view.leafRects.map((l, i) => LeafG(l, i, true))
            ) : (
              view.cols.map((col, ci) => (
                <g key={ci}>
                  ...
                  {col.headerH > 0 && (
                    <g style={{ cursor: col.kind === "sec" ? "pointer" : "default" }} onClick={() => col.kind === "sec" && setFocus(col.secName!)}
                      onMouseMove={e => col.kind === "sec" && showTip(e, [col.secName!, `${(col.secSize! / 1024).toFixed(2)} KB`, `${pct(col.secSize!, totalAll)}% of binary`, `${(data.find(d => d.name === col.secName)?.children || []).filter((c: any) => c.size > 0).length} functions`])}
                  ...
                  {col.kind === "sec"
                    ? col.leafRects.map((l, li) => LeafG(l, li, false))
                    : col.rows.map((rw, ri) => (
                      <g key={ri} className="tm-leaf" style={{ cursor: "pointer", opacity: hover && hover !== `row:${rw.name}` ? 0.45 : 1, transition: "opacity .12s" }}
                        onClick={() => setFocus(rw.name)}
                        onMouseEnter={() => setHover(`row:${rw.name}`)} onMouseLeave={() => { setHover(null); setTip(null); }}
                        onMouseMove={e => showTip(e, [rw.name, `${(rw.size / 1024).toFixed(2)} KB`, `${pct(rw.size, totalAll)}% of binary`, "click to zoom"])}>
      ...
      {tip && (
        <div style={{ position: "fixed", left: tip.x + 14, top: tip.y + 14, zIndex: 60, pointerEvents: "none", background: "var(--panel)", border: "1px solid var(--line2)", borderRadius: 4, padding: "7px 10px", fontFamily: "JetBrains Mono", fontSize: 11, lineHeight: 1.5, boxShadow: "0 10px 30px -12px rgba(0,0,0,.8)" }}>
          <div className="fg" style={{ fontWeight: 700 }}>{tip.lines[0]}</div>
          {tip.lines.slice(1).map((l, i) => <div key={i} className="mut">{l}</div>)}
        </div>
      )}
    </div>
  );
