...
const renderCell = (props: any) => {
  ...
  const isSection = !!p.children;            // top-level region vs function leaf
  const label = nm.length > 16 ? nm.slice(0, 14) + ".." : nm;

  return (
    <g>
      <rect x={x} y={y} width={width} height={height}
        fill={fill} stroke={stroke} strokeWidth={isSection ? 1.6 : 1} />
      <text x={x + width / 2} y={y + height / 2} textAnchor="middle"
        fill="#d8e1ec" fontSize={width > 110 ? 12 : 10}
        fontFamily="JetBrains Mono" fontWeight={700}>{label}</text>
      <text x={x + width / 2} y={y + height / 2 + 14} textAnchor="middle"
        fill={sub} fontSize={9} fontFamily="JetBrains Mono">{(sz / 1024).toFixed(1)} KB</text>
    </g>
  );
};
...
const Tip = ({ active, payload }: any) => {
  if (active && payload && payload.length && payload[0].payload) {
    ...
    return (
      <div className="mono text-[11px] px-3 py-2 rounded-[3px] border"
        style={{ background: "var(--panel)", borderColor: "var(--line2)" }}>
      ...
      <div className="p-3">
        <div className="h-[380px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <Treemap data={mapped} dataKey="size" aspectRatio={4 / 3}
