...
export default function Uploader({ onUpload, loading }: { onUpload: (f: File) => void; loading: boolean }) {
  ...
  return (
    <div className={`bay ${hot ? "hot" : ""} cursor-pointer px-6 py-10 text-center`}
      ...
      <div className="flex items-center justify-center gap-2 mono text-[11px] uppercase tracking-[.2em]">
        ...
        <span className={loading ? "acc2" : hot ? "acc" : "mut"}>
          {loading ? "parsing binary" : hot ? "link established" : "awaiting binary"}
        </span>
      </div>
