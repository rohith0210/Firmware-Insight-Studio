...
import { DB, DB_ORDER, VENDORS, fmt, getArchitectureProfile } from "../utils/devices";
...
export default function DeviceExplorer({
  ...
  const architectureProfile = useMemo(() => getArchitectureProfile(activeDevice), [activeDevice]);

  // Boot sequence based on selected MCU
  const getBootSteps = (): BootStep[] => {
    ...
    } else if (architectureProfile.isCortexM) {
      return [
    ...
    return [
      { title: "Firmware Entry", desc: "Execution begins at the ELF entry point defined by the selected binary and its platform runtime.", addr: result?.entry || "Not available" },
      { title: "Runtime Initialization", desc: "The toolchain initializes writable data and runtime state according to this target's ABI and linker script." },
      { title: "Application Entry", desc: "Control transfers to the application entry routine provided by the firmware." },
    ];
  };

  const baseVector = activeDevice.vectorTableAddr || 0x08000000;
  const vectors = architectureProfile.isCortexM ? [
    ...
    <div className="p-6 h-full overflow-y-auto space-y-6 bg-[var(--bg)] text-white mono text-xs select-none">
      ...
      <div className="p-5 rounded-b-xl bg-black/30 border border-[var(--line)] min-h-[300px]">
        {activeTab === "overview" && (
          <div className="space-y-6">
            <div className="grid grid-cols-4 gap-4">
              ...
              <div className="p-4 rounded-xl bg-black/40 border border-[var(--line)] space-y-1">
                <div className="text-[10px] text-[var(--mut)] uppercase font-bold">Interrupt Model</div>
                <div className="text-base font-bold text-purple-400 font-mono">{architectureProfile.isCortexM ? `0x${(activeDevice.vectorTableAddr || 0x08000000).toString(16)}` : "Platform-defined"}</div>
                <div className="text-[10px] text-gray-400">{architectureProfile.isCortexM ? `${activeDevice.interruptCount || 48} IRQ vectors` : architectureProfile.interruptModel}</div>
              </div>
              <div className="p-4 rounded-xl bg-black/40 border border-[var(--line)] space-y-1">
                <div className="text-[10px] text-[var(--mut)] uppercase font-bold">Default Toolchain</div>
                <div className="text-sm font-bold text-amber-300">{activeDevice.defaultToolchain || "Toolchain not declared"}</div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-start">
              ...
              <span className="px-2.5 py-1 rounded bg-[var(--a-dim)] text-[var(--a)] border border-[var(--a)] font-bold text-xs">
                {architectureProfile.bits}-Bit {activeDevice.architecture} Architecture
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-xl bg-black/40 border border-[var(--line)] space-y-1">
                ...
                <div className="text-white font-mono text-xs">{architectureProfile.registerModel}</div>
              </div>
              <div className="p-4 rounded-xl bg-black/40 border border-[var(--line)] space-y-1">
                <div className="text-[10px] text-[var(--mut)] uppercase font-bold">Calling Convention</div>
                <div className="text-amber-300 font-mono text-xs">{architectureProfile.callingConvention}</div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-black/40 border border-[var(--line)] space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-gray-200 text-xs">
                {architectureProfile.capabilities.map(capability => <div key={capability} className="p-2 rounded bg-black/50 border border-[var(--line)]">✓ {capability}</div>)}
              </div>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody>
                ...
                {!architectureProfile.isCortexM && <tr><td colSpan={5} className="py-6 text-center text-[var(--mut)]">{architectureProfile.interruptModel}. This target does not use the Cortex-M NVIC vector layout.</td></tr>}
              </tbody>
            </table>
          </div>
        )}
        ...
        {activeTab === "peripherals" && (
          <div className="space-y-4">
            <h3 className="text-xs font-bold text-white uppercase">Peripheral Register Model</h3>
            {architectureProfile.isCortexM ? <div className="grid grid-cols-3 gap-3">
              {["GPIO", "USART", "SPI", "I2C", "CAN", "USB", "DMA", "ADC", "DAC", "TIMERS", "RCC", "NVIC"].map(p => (
                <div key={p} className="p-3 rounded-lg bg-black/40 border border-[var(--line)] space-y-1">
                  ...
                  <div className="text-[10px] text-gray-400">Family-level capability; confirm addresses in the device reference manual.</div>
                </div>
              ))}
            </div> : <div className="rounded-lg border border-[var(--line)] bg-black/40 p-4 text-[11px] text-gray-300">No vendor-specific peripheral register map is bundled for {activeDevice.name}. The selected profile supplies the ISA and memory-region model only; use the device datasheet/reference manual for register addresses.</div>}
          </div>
        )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-xl bg-black/40 border border-[var(--line)] space-y-2">
                ...
                <div className="space-y-1 text-xs">
                  <div className="text-amber-300 font-mono">Primary: {activeDevice.defaultToolchain || "Not declared for this profile"}</div>
                  <div className="text-gray-300">LLVM / Clang target support depends on the selected architecture</div>
                </div>
              </div>
              <div className="p-4 rounded-xl bg-black/40 border border-[var(--line)] space-y-2">
                ...
                <div className="space-y-1 text-xs text-emerald-400 font-mono">
                  {architectureProfile.isCortexM ? <><div>✓ SWD (Serial Wire Debug)</div><div>✓ JTAG (IEEE 1149.1)</div><div>✓ OpenOCD-compatible debug probe</div></> : <div>Interface is platform and board specific; no ARM debug interface is assumed.</div>}
                </div>
              </div>
            </div>
          </div>
