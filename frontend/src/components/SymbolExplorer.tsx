  ...
  return (
    <div className="panel">
      <div className="panel-head">
        <span>{title}</span>
        <span className="tag">{filtered.length} matches · click row to inspect</span>
      </div>
      <div className="p-3">
        <div className="flex items-center gap-2 mb-3">
          <div className="flex items-center gap-2 flex-1 border ln rounded-[3px] px-3 py-2 bg-black/30">
            <span className="acc mono">›</span>
            <input
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="grep symbol  e.g. main, HAL_GPIO"
              className="flex-1 bg-transparent outline-none mono text-[12px] fg placeholder:mut"
            />
          </div>
          <div className="selbar">
            <span className="mono text-[10px] mut">sort</span>
            ...
            <tbody>
              {filtered.slice(0, limit).map((s, i) => (
                <tr
                  ...
                  className="sym-row border-b ln hover:bg-[rgba(51,214,194,0.06)] cursor-pointer transition-colors"
                >
                  <td className="py-1.5 acc font-bold">{s.name}</td>
        ...
        {filtered.length > limit && (
          <button onClick={() => setLimit(limit + 60)} className="btn-hw mt-3">
            load +60 ({filtered.length - limit} left)
          </button>
        )}
      </div>
    </div>
