          ...
          <div className="flex items-center gap-2">
            {result && (
              <div className="mono text-[11px] mut hidden md:flex items-center gap-3 px-3 py-2 border ln rounded-[3px] bg-black/30">
                <span className="fg">{result.filename}</span>
                <span className="acc">{result.arch}</span>
                <span>{result.symbols.length} sym</span>
              </div>
            )}
        ...
        {result && secNames.length > 0 && (
          <div className="ticker mt-4">
            <div className="ticker-track">
              {[...secNames, ...secNames].map((n, i) => <span key={i}><b>//</b>{n}</span>)}
            </div>
          </div>
        )}
