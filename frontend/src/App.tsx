...
export default function App() {
  ...
  const handleUpload = async (file: File) => {
    setLoading(true); setError(null); setSelectedSection(null); setResultB(null); setDeviceOverride(""); setView("overview");
  ...
  const exportJSON = () => result && download(new Blob([JSON.stringify({ base: result, candidate: resultB, device: device?.name }, null, 2)], { type: "application/json" }), `${result.filename}_analysis.json`);
  ...
  const renderView = () => {
    ...
    switch (view) {
      ...
      case "linker": return <LinkerScript result={result} device={device} />;
      ...
      <div className="shell">
        ...
        <div className="main">
          <Ribbon title={TITLES[view]} result={result} loading={loading} accent={accent}
            device={device} override={deviceOverride} setOverride={setDeviceOverride}
            cycleAccent={() => setAccent(a => a === "signal" ? "phosphor" : "signal")}
            onJSON={exportJSON} onCSV={exportCSV} onReset={() => { setResult(null); setResultB(null); setDeviceOverride(""); }} />
...
function DevicePicker({ device, override, setOverride }: { device: any; override: string; setOverride: (s: string) => void }) {
  return (
    <span className="chip" title="detected target — click to override">
      <span className="mut">target</span>
      <select value={override} onChange={e => setOverride(e.target.value)} style={{ background: "transparent", border: 0, color: "var(--a)", fontFamily: "JetBrains Mono", fontSize: 11, outline: "none" }}>
        <option value="">auto · {device?.name || "—"}</option>
        {DB_ORDER.map(id => <option key={id} value={id}>{DB[id].name}</option>)}
        <option value="generic">Cortex-M · generic</option>
      </select>
    </span>
  );
}
  ...
  return (
    <div className="panel">
      ...
      <div className="p-5 grid sm:grid-cols-2 gap-4">
        <button onClick={onJSON} className="btn-hw primary text-left !py-4 !px-4 flex flex-col items-start gap-1"><span className="text-base">⎙ Full Analysis · JSON</span><span className="mono text-[10px] mut normal-case tracking-normal">sections · symbols · memory · graph · dead code · device</span></button>
  ...
  return (
    <div className="panel">
      ...
      <div className="p-5 grid sm:grid-cols-2 gap-3 max-w-2xl">
        ...
        <label className="flex flex-col gap-2 p-3 border ln rounded-[3px] sm:col-span-2">
          <span className="mono text-[12px] fg">Target device override</span>
          <select value={override} onChange={e => setOverride(e.target.value)} className="selbar" style={{ width: "100%" }}>
            <option value="">auto-detect · {device?.name || "—"}</option>
            {DB_ORDER.map(id => <option key={id} value={id}>{DB[id].name}</option>)}
            <option value="generic">Cortex-M · generic</option>
          </select>
          <span className="mono text-[10px] mut">auto-detect reads arch + .text/.data base addresses; override for exact FLASH/RAM capacity.</span>
        </label>
      </div>
    </div>
  );
