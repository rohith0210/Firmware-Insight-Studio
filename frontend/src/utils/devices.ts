...
const minAddr = (secs: any[], names: string[]) => { const m = secs.filter(s => names.includes(s.name) && s.size > 0).map(s => s.addr >>> 0); return m.length ? Math.min(...m) : 0; };
export function detectDevice(res: { arch: string; elf_class?: number; sections: any[]; summary: Record<string, number> }): Device {
  ...
  if (/x86|80386|i386|amd64/i.test(arch)) { const t = minAddr(res.sections, [".text", ".init", ".rodata"]) || 0x1000; const d = minAddr(res.sections, [".data", ".bss"]) || 0x4000; return { id: "x86", name: `${arch} · host binary`, mcu: false, regions: [r(".text/.rodata", "virt", t, usedF || 1), r(".data/.bss", "virt", d, (res.summary[".data"] || 0) + (res.summary[".bss"] || 0) || 1)] }; }
  if (/aarch64|arm64/i.test(arch)) { const t = minAddr(res.sections, [".text", ".rodata"]) || 0x40000; const d = minAddr(res.sections, [".data", ".bss"]) || 0x80000; return { id: "aarch64", name: "AArch64 · host/generic", mcu: false, regions: [r("code", "virt", t, usedF || 1), r("data", "virt", d, (res.summary[".data"] || 0) + (res.summary[".bss"] || 0) || 1)] }; }
  const t = (minAddr(res.sections, [".text", ".init", ".rodata", ".isr_vector"]) || 0) >>> 0; const top = t & 0xff000000;
  if (top === 0x08000000) { if (usedF <= 64 * K) return DB.stm32f103c8; if (usedF <= 256 * K) return DB.stm32f303; if (usedF <= 1024 * K) return DB.stm32f407; return DB.stm32h743; }
  ...
  return { id: "generic", name: "Cortex-M · generic", mcu: true, regions: [r("FLASH", "flash", fb, np2(usedF) * 2 || 64 * K), r("SRAM", "ram", db & 0xfff00000, np2((res.summary[".data"] || 0) + (res.summary[".bss"] || 0)) * 2 || 16 * K)] };
}
...
export const usedIn = (d: Device, secs: any[], kinds: Region["kind"][]) => { const rgs = d.regions.filter(rg => kinds.includes(rg.kind)); return secs.filter(s => s.size > 0 && rgs.some(rg => inRegion(rg, s.addr))).reduce((a, s) => a + s.size, 0); };
export function fmt(b: number): string { if (b >= M) { const mb = b / M; return (Number.isInteger(mb) ? mb : mb.toFixed(1)) + "M"; } if (b >= K) { const kb = b / K; return (Number.isInteger(kb) ? kb : kb.toFixed(1)) + "K"; } return b + "B"; }
